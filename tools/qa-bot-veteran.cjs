...
const MAX_FLOOR = +(process.argv[3] || 55);
...
(async () => {
  ...
  await page.evaluate(() => {
    ...
    /* A VETERAN: every skill-tree node maxed, deep prestige, a pet, all tiers open. */
    ZB.META.tree = {};
    ZB.TREE.forEach(br => br.nodes.forEach(n => { ZB.META.tree[n.id] = n.rep ? 12 : 1; }));
    ZB.META.marks = 20;
    ZB.META.migrations = 4;
    ZB.META.stats = ZB.META.stats || {};
    ZB.META.stats.floors = 40;
    ZB.META.tiers = ['ed','seal','mult','glass'];
    ZB.META.pets = ZB.PETS.map(p => p.id);
    ZB.META.petSel = ZB.PETS[ZB.PETS.length - 1].id;
    ZB.META.stripes = 50000;

...
const ACE_Map = (function () {
  ...
  function drawModelOverlay(info) {
    ...
    function draw3dBuilding(lat, lng, w, h, prog, color) {
      ...
      var bw = w * sc, fullH = h * sc;
      // Ghost outline always
      ctx.strokeStyle = color;
      ctx.globalAlpha = 0.25;
      ctx.lineWidth = 1.5;
      ctx.setLineDash([4, 3]);
      ctx.strokeRect(p.x - bw / 2, p.y - fullH, bw, fullH);
      ctx.setLineDash([]);
      ctx.globalAlpha = 0.75;
      // Fill with progress
      if (prog > 0.01) {
      ...
      ctx.fillStyle = color;
      ctx.globalAlpha = 0.7;
      ctx.font = Math.max(9, 10 * sc) + 'px IBM Plex Mono, monospace';
      ctx.textAlign = 'center';
      ctx.fillText(Math.round(prog * 100) + '%', p.x, p.y + 12 * sc);
      ctx.globalAlpha = 0.75;
    }

    // Reactor
    draw3dBuilding(44.3265, -81.5983, 40, 80, progress.reactor, '#a8401f');
    // Containment dome (ghost + fill)
    var rp = toPixel(44.3265, -81.5983);
    var fullDomeR = 22 * sc;
    ctx.strokeStyle = 'rgba(168,64,31,.25)'; ctx.lineWidth = 1.5; ctx.setLineDash([4,3]);
    ctx.beginPath(); ctx.arc(rp.x, rp.y - 80 * sc, fullDomeR, Math.PI, 0); ctx.stroke();
    ctx.setLineDash([]);
    if (progress.containment > 0.05) {
      var domeR = fullDomeR * progress.containment;
      var domeY = rp.y - 80 * sc * Math.max(0.01, progress.reactor);
    ...
    // Cooling towers (ghost + fill)
    [[44.3275, -81.5960], [44.3275, -81.5945]].forEach(function(ll) {
      var cp = toPixel(ll[0], ll[1]);
      var ch2Full = 60 * sc;
      // Ghost
      ctx.strokeStyle = 'rgba(107,76,154,.2)'; ctx.lineWidth = 1; ctx.setLineDash([4,3]);
      ctx.beginPath();
      ctx.moveTo(cp.x - 18 * sc, cp.y); ctx.lineTo(cp.x - 12 * sc, cp.y - ch2Full);
      ctx.lineTo(cp.x + 12 * sc, cp.y - ch2Full); ctx.lineTo(cp.x + 18 * sc, cp.y);
